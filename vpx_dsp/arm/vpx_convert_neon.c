...
void RGB24_to_YV12_bt701_patch_neon(YV12_BUFFER_CONFIG *ybf, RGB24_BUFFER_CONFIG *rbf, int src_x_offset, int src_y_offset, int dst_x_offset, int dst_y_offset, int width, int height) {
    uint8_t r, g, b;
    uint8x8_t ry_coeff = vdup_n_u8(RY_COEFF_INT);
    uint8x8_t gy_coeff = vdup_n_u8(GY_COEFF_INT);
    uint8x8_t by_coeff = vdup_n_u8(BY_COEFF_INT);
    ...
    for (i = 0; i < height; i++) {
        for (j = 0; j <= width - 16; j += 16) {
            rgb_n = vld3_u8(rbf->buffer_alloc + (i+src_y_offset) * rbf->stride + (j+src_x_offset) * 3);
            rgb1_n = vld3_u8(rbf->buffer_alloc + (i+src_y_offset) * rbf->stride + ((j+src_x_offset) + 8) * 3);
            ...
            vst1_u8(ybf->y_buffer + (i+dst_y_offset) * ybf->y_stride + (j+dst_x_offset), y_n);
            vst1_u8(ybf->y_buffer + (i+dst_y_offset) * ybf->y_stride + (j+dst_x_offset) + 8, y1_n);
        }

        for (; j < width; j++) {
            r = *(rbf->buffer_alloc + (i+src_y_offset) * rbf->stride + (j+src_x_offset) * 3);
            g = *(rbf->buffer_alloc + (i+src_y_offset) * rbf->stride + (j+src_x_offset) * 3 + 1);
            b = *(rbf->buffer_alloc + (i+src_y_offset) * rbf->stride + (j+src_x_offset) * 3 + 2);
            *(ybf->y_buffer + (i+dst_y_offset) * ybf->y_stride + (j+dst_x_offset)) = (uint8_t) (((RY_COEFF_INT * r + GY_COEFF_INT * g + BY_COEFF_INT * b + CONVERT_DELTA) >> CONVERT_FRACTION_BIT) +
                                                                  Y_OFFSET);
        }
    }
    ...
    for (i = 0; i < height; i = i + (1 << ybf->subsampling_y)) {
        for (j = 0; j <= width - (16 << ybf->subsampling_x); j = j + (16 << ybf->subsampling_x)) {
            rgb_n = vld3_lane_u8(rbf->buffer_alloc + (i+src_y_offset) * rbf->stride + (j+src_x_offset)* 3, rgb_n, 0);
            rgb_n = vld3_lane_u8(rbf->buffer_alloc + (i+src_y_offset) * rbf->stride + ((j+src_x_offset) + (1 << ybf->subsampling_x)) * 3, rgb_n, 1);
            rgb_n = vld3_lane_u8(rbf->buffer_alloc + (i+src_y_offset) * rbf->stride + ((j+src_x_offset) + (2 << ybf->subsampling_x)) * 3, rgb_n, 2);
            rgb_n = vld3_lane_u8(rbf->buffer_alloc + (i+src_y_offset) * rbf->stride + ((j+src_x_offset) + (3 << ybf->subsampling_x)) * 3, rgb_n, 3);
            rgb_n = vld3_lane_u8(rbf->buffer_alloc + (i+src_y_offset) * rbf->stride + ((j+src_x_offset) + (4 << ybf->subsampling_x)) * 3, rgb_n, 4);
            rgb_n = vld3_lane_u8(rbf->buffer_alloc + (i+src_y_offset) * rbf->stride + ((j+src_x_offset) + (5 << ybf->subsampling_x)) * 3, rgb_n, 5);
            rgb_n = vld3_lane_u8(rbf->buffer_alloc + (i+src_y_offset) * rbf->stride + ((j+src_x_offset) + (6 << ybf->subsampling_x)) * 3, rgb_n, 6);
            rgb_n = vld3_lane_u8(rbf->buffer_alloc + (i+src_y_offset) * rbf->stride + ((j+src_x_offset) + (7 << ybf->subsampling_x)) * 3, rgb_n, 7);
            rgb1_n = vld3_lane_u8(rbf->buffer_alloc + (i+src_y_offset) * rbf->stride + ((j+src_x_offset) + (8 << ybf->subsampling_x)) * 3, rgb1_n, 0);
            rgb1_n = vld3_lane_u8(rbf->buffer_alloc + (i+src_y_offset) * rbf->stride + ((j+src_x_offset) + (9 << ybf->subsampling_x)) * 3, rgb1_n, 1);
            rgb1_n = vld3_lane_u8(rbf->buffer_alloc + (i+src_y_offset) * rbf->stride + ((j+src_x_offset) + (10 << ybf->subsampling_x)) * 3, rgb1_n, 2);
            rgb1_n = vld3_lane_u8(rbf->buffer_alloc + (i+src_y_offset) * rbf->stride + ((j+src_x_offset) + (11 << ybf->subsampling_x)) * 3, rgb1_n, 3);
            rgb1_n = vld3_lane_u8(rbf->buffer_alloc + (i+src_y_offset) * rbf->stride + ((j+src_x_offset) + (12 << ybf->subsampling_x)) * 3, rgb1_n, 4);
            rgb1_n = vld3_lane_u8(rbf->buffer_alloc + (i+src_y_offset) * rbf->stride + ((j+src_x_offset) + (13 << ybf->subsampling_x)) * 3, rgb1_n, 5);
            rgb1_n = vld3_lane_u8(rbf->buffer_alloc + (i+src_y_offset) * rbf->stride + ((j+src_x_offset) + (14 << ybf->subsampling_x)) * 3, rgb1_n, 6);
            rgb1_n = vld3_lane_u8(rbf->buffer_alloc + (i+src_y_offset) * rbf->stride + ((j+src_x_offset) + (15 << ybf->subsampling_x)) * 3, rgb1_n, 7);
            ...
            vst1_u8(ybf->u_buffer + ((i+dst_y_offset) >> ybf->subsampling_y) * ybf->uv_stride + ((j+dst_x_offset) >> ybf->subsampling_x), u_n);
            vst1_u8(ybf->v_buffer + ((i+dst_y_offset) >> ybf->subsampling_y) * ybf->uv_stride + ((j+dst_x_offset) >> ybf->subsampling_x), v_n);
            vst1_u8(ybf->u_buffer + ((i+dst_y_offset) >> ybf->subsampling_y) * ybf->uv_stride + ((j+dst_x_offset) >> ybf->subsampling_x) + 8, u1_n);
            vst1_u8(ybf->v_buffer + ((i+dst_y_offset) >> ybf->subsampling_y) * ybf->uv_stride + ((j+dst_x_offset) >> ybf->subsampling_x) + 8, v1_n);
        }
        ...
        for (; j < width; j++) {
            ...
            g = *(rbf->buffer_alloc + (i+src_y_offset) * rbf->stride + (j+src_x_offset)* 3 + 1);
            b = *(rbf->buffer_alloc + (i+src_y_offset) * rbf->stride + (j+src_x_offset)* 3 + 2);
            *(ybf->u_buffer + ((i+dst_y_offset) >> ybf->subsampling_y) * ybf->uv_stride + ((j+dst_x_offset) >> ybf->subsampling_x)) = (uint8_t) (
                    ((-RU_COEFF_INT * r - GU_COEFF_INT * g + BU_COEFF_INT * b + CONVERT_DELTA) >> CONVERT_FRACTION_BIT) + U_OFFSET);
            *(ybf->v_buffer + ((i+dst_y_offset) >> ybf->subsampling_y) * ybf->uv_stride + ((j+dst_x_offset) >> ybf->subsampling_x)) = (uint8_t) (
                    ((RV_COEFF_INT * r - GV_COEFF_INT * g - BV_COEFF_INT * b + CONVERT_DELTA) >> CONVERT_FRACTION_BIT) + V_OFFSET);
        }
    }
}
...
int RGB24_to_YV12_patch_neon(YV12_BUFFER_CONFIG *ybf, RGB24_BUFFER_CONFIG *rbf, vpx_color_space_t color_space, vpx_color_range_t color_range, int src_x_offset, int src_y_offset, int dst_x_offset, int dst_y_offset, int width, int height) {
    if (ybf == NULL || rbf == NULL) {
        return -1;
    }

    if (color_space == VPX_CS_BT_709 && color_range == VPX_CR_STUDIO_RANGE) {
        RGB24_to_YV12_bt701_patch_neon(ybf, rbf, src_x_offset, src_y_offset, dst_x_offset, dst_y_offset, width, height);
    }
    else {
        return -1;
    }
