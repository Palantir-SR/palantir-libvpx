...
static const arg_def_t savesuperfinegrainedmetadataarg = ARG_DEF(NULL, "save-super-finegrained-metadata", 0, "Save a super-finegrained metadata log");
static const arg_def_t savefinegrainedmetadataarg =
        ARG_DEF(NULL, "save-finegrained-metadata", 0, "Save a fine-grained metadata log");
...
static const arg_def_t npatchespercolarg =
        ARG_DEF(NULL, "npatches-per-column", 1, "number of patches per column");
static const arg_def_t patchwidtharg =
        ARG_DEF(NULL, "patch-width", 1, "the size of the patch");
static const arg_def_t patchheightarg =
        ARG_DEF(NULL, "patch-height", 1, "the size of the patch");
static const arg_def_t save_frame_size_arg = 
        ARG_DEF(NULL, "save-metadata-framesize", 0, "Save the frame size to the metadata log"); 
...
static void setup_directory(palantir_cfg_t *palantir_cfg, const char *dataset_dir, const char *input_video_name, const char *reference_video_name,
                                const char *dnn_name, const char *cache_profile_name, const char *postfix) {
    switch(palantir_cfg->decode_mode) {
        case DECODE:
            sprintf(palantir_cfg->log_dir, "%s/log/%s", dataset_dir, input_video_name);
            sprintf(palantir_cfg->input_frame_dir, "%s/image/%s", dataset_dir, input_video_name);
            append_postfix_to_string(palantir_cfg->log_dir, postfix);
            append_postfix_to_string(palantir_cfg->input_frame_dir, postfix);

            if (palantir_cfg->save_quality) {
                sprintf(palantir_cfg->input_reference_frame_dir, "%s/image/%s", dataset_dir, reference_video_name);
                append_postfix_to_string(palantir_cfg->input_reference_frame_dir, postfix);
            }
            break;
        case DECODE_SR:
            sprintf(palantir_cfg->log_dir, "%s/log/%s/%s", dataset_dir, input_video_name, dnn_name);
            sprintf(palantir_cfg->input_frame_dir, "%s/image/%s", dataset_dir, input_video_name);
            sprintf(palantir_cfg->sr_frame_dir, "%s/image/%s/%s", dataset_dir, input_video_name, dnn_name);
            sprintf(palantir_cfg->sr_offline_frame_dir, "%s/image/%s/%s", dataset_dir, input_video_name, dnn_name);
            append_postfix_to_string(palantir_cfg->log_dir, postfix);
            append_postfix_to_string(palantir_cfg->input_frame_dir, postfix);
            append_postfix_to_string(palantir_cfg->sr_frame_dir, postfix);
            append_postfix_to_string(palantir_cfg->sr_offline_frame_dir, postfix);

            if (palantir_cfg->save_quality) {
                sprintf(palantir_cfg->input_reference_frame_dir, "%s/image/%s", dataset_dir, input_video_name);
                sprintf(palantir_cfg->sr_reference_frame_dir, "%s/image/%s", dataset_dir, reference_video_name);
                append_postfix_to_string(palantir_cfg->input_reference_frame_dir, postfix);
                append_postfix_to_string(palantir_cfg->sr_reference_frame_dir, postfix);
            }
            break;
        case DECODE_CACHE:case DECODE_BLOCK_CACHE:
            ...
            if (cache_profile_name != NULL) {
                append_postfix_to_string(palantir_cfg->log_dir, cache_profile_name);
                append_postfix_to_string(palantir_cfg->sr_frame_dir, cache_profile_name);
            }
            else {
                append_postfix_to_string(palantir_cfg->log_dir, "FAST");
                append_postfix_to_string(palantir_cfg->sr_frame_dir, "FAST");
            }
...
static void set_decode_mode(palantir_decode_mode *decode_mode, const char *decode_mode_str)
{
    ...
    else if (strcmp(decode_mode_str, "decode_block_cache") == 0) {
...
static void set_dnn_mode(palantir_dnn_mode *dnn_mode, const char *dnn_mode_str)
{
...
static void set_cache_mode(palantir_cache_mode *cache_mode, const char *cache_mode_str)
{
...
static void set_runtime(palantir_dnn_runtime *dnn_runtime, const char *dnn_runtime_str)
{
...
static int main_loop(int argc, const char **argv_)
{
  ...
  vpx_image_t *img_shifted = NULL;
    ...
    for (argi = argj = argv; (*argj = *argi); argi += arg.argv_step)
    {
      ...
      ec_enabled = 1;
        ...
        /* PALANTIR: new arguments */
            ...
            palantir_cfg->target_width = atoi(arg.val);
        else if (arg_match(&arg, &outputheightarg, argi))
            palantir_cfg->target_height = atoi(arg.val);
        ...
        else if (arg_match(&arg, &savesuperfinegrainedmetadataarg, argi))
            palantir_cfg->save_super_finegrained_metadata = 1;
        else if (arg_match(&arg, &savefinegrainedmetadataarg, argi))
            palantir_cfg->save_finegrained_metadata = 1;
        else if (arg_match(&arg, &saveyuvframearg, argi))
            palantir_cfg->save_yuvframe = 1;
            ...
            palantir_cfg->num_patches_per_column = atoi(arg.val);
        else if (arg_match(&arg, &patchwidtharg, argi))
            palantir_cfg->patch_width = atoi(arg.val);
        else if (arg_match(&arg, &patchheightarg, argi))
            palantir_cfg->patch_height = atoi(arg.val);
        else if (arg_match(&arg, &save_frame_size_arg, argi))
            palantir_cfg->save_frame_size = 1;
        else
            argj++;
    }
    ...
    if (palantir_cfg->save_finegrained_metadata || palantir_cfg->save_super_finegrained_metadata) {
        fprintf(stdout, "Note: when save_(super_)finegrained_metadata is enabled, the number of threads must be set to 1");
        assert(cfg.threads==1);
        fprintf(stdout, "Note: when save_(super_)finegrained_metadata is enabled, the decode mode must be set to DECODE_BLOCK_CACHE");
        assert(palantir_cfg->decode_mode==DECODE_BLOCK_CACHE);
    }
    setup_directory(palantir_cfg, dataset_dir, input_video_name, reference_video_name, dnn_name, cache_profile_name, postfix);
    ...
    if (palantir_cfg->decode_mode == DECODE_SR || palantir_cfg->decode_mode == DECODE_CACHE || palantir_cfg->decode_mode == DECODE_BLOCK_CACHE) {
        ...
        if (palantir_cfg->decode_mode == DECODE_BLOCK_CACHE) {
            sprintf(palantir_cfg->dnn_dir, "%s/checkpoint/%s/%s_block_%d_%d.dlc", dataset_dir, input_video_name, dnn_name, palantir_cfg->patch_width, palantir_cfg->patch_height);
        } else {
            sprintf(palantir_cfg->dnn_dir, "%s/checkpoint/%s/%s_frame.dlc", dataset_dir, input_video_name, dnn_name);
        }
    }
    if ((palantir_cfg->decode_mode == DECODE_CACHE || palantir_cfg->decode_mode == DECODE_BLOCK_CACHE) && palantir_cfg->cache_mode == PROFILE_CACHE) {
        if (postfix == NULL) {
  ...
  if (vp8_pp_cfg.post_proc_flag &&
    ...
    /* PALANTIR: Load PALANTIR configuration */
    if (vpx_load_palantir_cfg(&decoder, palantir_cfg)){
         warn("Failed to load configuration: %s\n", vpx_codec_error(&decoder));
         goto fail;
     };

    /* PALANTIR: Load a DNN */
    if (palantir_cfg->dnn_mode == ONLINE_DNN && (palantir_cfg->decode_mode == DECODE_SR || palantir_cfg->decode_mode == DECODE_CACHE || palantir_cfg->decode_mode == DECODE_BLOCK_CACHE)) {
        //fprintf(stderr, "vpx_load_palantir_dnn %s/checkpoint/%s/%s.dlc", dataset_dir, input_video_name, dnn_name);
        if(vpx_load_palantir_dnn(&decoder, dnn_scale, palantir_cfg->dnn_dir)){
            warn("Failed to load a DNN: %s\n", vpx_codec_error(&decoder));
            goto fail;
        }
    }

    if (palantir_cfg->dnn_mode == OFFLINE_DNN && (palantir_cfg->decode_mode == DECODE_SR || palantir_cfg->decode_mode == DECODE_CACHE || palantir_cfg->decode_mode == DECODE_BLOCK_CACHE)) {
        if(vpx_load_palantir_dnn(&decoder, dnn_scale, NULL)){
            warn("Failed to load a DNN: %s\n", vpx_codec_error(&decoder));
            goto fail;
        }
    }

    /* PALANTIR: Load a cache profile */
    ...
    /* Palantir: Init the dependency graph */
    if (palantir_cfg->save_finegrained_metadata) {
        if(vpx_init_palantir_dependeny_graph(&decoder, palantir_cfg->num_patches_per_row, palantir_cfg->num_patches_per_column)){
            warn("Failed to load a cache profile: %s\n", vpx_codec_error(&decoder));
            goto fail;
        }
    }
